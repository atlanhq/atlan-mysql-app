...
def sample_dataframe():
    return daft.from_pydict(  # type: ignore
        {
...
def test_quote_column_name(sql_transformer: QueryBasedTransformer):
...
def test_convert_to_sql_expression(sql_transformer: QueryBasedTransformer):
...
def test_convert_to_sql_expression_with_literal(sql_transformer: QueryBasedTransformer):
...
def test_get_sql_column_expressions(
    ...
    assert len(literal_columns) == 1  # type: ignore
...
def test_build_struct(sql_transformer: QueryBasedTransformer):
    ...
    result = sql_transformer._build_struct(level, "test")  # type: ignore
...
def test_get_grouped_dataframe_by_prefix(sql_transformer: QueryBasedTransformer):
    """
    ...
    df = daft.from_pydict(  # type: ignore
        {
    ...
    result: daft.DataFrame = sql_transformer.get_grouped_dataframe_by_prefix(df)  # type: ignore
    assert result.count_rows() == 3  # type: ignore
    expected_schema = [
    ...
    assert expected_schema == [schema for schema in result.schema()]  # type: ignore
...
def test_transform_metadata_empty_dataframe(
    sql_transformer: QueryBasedTransformer,
):
    """Test transform_metadata with empty dataframe"""
    empty_df = daft.from_pydict(  # type: ignore
        {"dummy": []}
