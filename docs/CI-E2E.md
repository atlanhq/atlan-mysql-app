...
|---|---|---|
| `SDR_TEST_TENANT` | both | DNS short name (e.g. `devex` for `devex.atlan.com`) |
| `SDR_CLIENT_ID` / `SDR_CLIENT_SECRET` | both | Configurator OAuth credentials |
| `ATLAN_BASE_URL` | full-DAG | Full URL (e.g. `https://devex.atlan.com`) |
...
Same set used by `atlan-mssql-app` — copy values from there, or grab from `#bu-security-and-it`.
