...
async function testConnection() {
  ...
  try {
    ...
    // Show error message with MySQL-specific guidance
    errorElement.textContent =
      error.message ||
      "Failed to connect to MySQL server. Please check your credentials and ensure the server is accessible.";
...
function processMetadataResponse(data) {
  // For MySQL, we handle database and schema differently
...
function populateDropdown(type, databases) {
  ...
  // MySQL system schemas that should be handled differently
  ...
  databases.forEach((schemas, tableCatalog) => {
    ...
    // Add system class for system schemas
    if (systemSchemas.includes(tableCatalog)) {
      dbContainer.classList.add("system-schema");
    }
    ...
    // Add visual indicator for system schemas
    if (systemSchemas.includes(tableCatalog)) {
      label.innerHTML = `${tableCatalog} <span class="badge system-badge">system</span>`;
    }
